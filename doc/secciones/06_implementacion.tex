...
Teniendo desarrollada toda la funcionalidad que permite extraer todos los datos
...
buscar URLs relevantes, construir el objeto \texttt{Page} correcto y almacenarlo
en la entidad \texttt{TvTropesPages}, sin realizar ninguna tarea de extracción
de datos, ya que no es su cometido. El \textit{scraper} únicamente necesita el
...
El \textit{crawler} debe manterse simple en su implementación, para dar una
...
lugares; se pueden encontrar nuevas fuentes de datos o páginas relevantes que
...
de conseguir todas las páginas relevantes de TvTropes de la forma más eficiente
posible, ya que será la parte de todo el \textit{software} que tratará con un
mayor volúmen de datos. Adicionalmente, se deben tener muy en cuenta los
aspectos éticos y de eficiencia del \textit{scraping}, ya que en este punto del
...
y encapsulada que pueda trabajar de forma individual, sin embargo, su último fin
...
usuarios. El \textit{crawler} primero buscará e indexará todas las páginas
relevantes en una entidad común, utilizable por cualquier cliente del
\textit{crawler}. Ese cliente será el \textit{scraper}, que usará esas páginas
para extraer los datos y generar el \textit{dataset} con el resultado final. 
...
buen punto de partida y cómo desde ahí encontrar exactamente el tipo de páginas
...
proceso de extracción. En nuestro caso, el principal artefacto son las películas
(o, en general, cualquier obra), ya que es de dónde luego el \textit{scraper}
extraerá toda su información: metadatos y \textit{tropos}.

En la figura \ref{fig:crawling} se puede ver un diagrama de flujo que detalla la
estrategia que se sigue para encontrar y extraer todas las páginas relevantes de
TvTropes; en este primer caso, todas las películas, pero en el próximo hito se
ampliará para cualquier otro medio audiovisual que presente obras con
\textit{tropos}. Esta es la estrategia que sigue una araña en cualquier ámbito
\cite{olston2010web}, sin embargo, para el caso de este problema en concreto se
tienen en cuenta las siguientes consideraciones adicionales:
\begin{itemize}
    \item La estrategia de extracción en TvTropes en este incremento consiste en
    la indexación de páginas y sub páginas de obra, es decir, aquellas que están
    ...
    principal de la obra y/o en cualquiera de sus sub páginas.
    ...
    \item Una vez extraídas y almacenadas todas las URLs en una variable
    intermedia, se construyen todos los objetos \texttt{Page} y la entidad \texttt{TvTropesPages} final 
    que conformará el resultado del \textit{crawling}.
\end{itemize}
...
anteriormente no siempre es cierta, porque aunque técnicamente estén
...
sólo extrae los que están en la sección principal, ignorando las sub wikis.
...
relación entre el bot y la página web.

En base a las \href{https://github.com/jlgallego99/TropesToGo/issues/7}{[HU02]} y \href{https://github.com/jlgallego99/TropesToGo/issues/45}{[HU06]}, que nos indican, se permite limitar el proceso de \textit{crawling} a un número de obras limitado en lugar de encontrarlas todas.

\subsubsection{Caché/persistencia de las páginas scrapeadas}
Usar punteros a los Goquery documents.
\subsubsection{Limitación de la tasa de crawling}
Esperas aleatorias entre petición y petición.

Todo esto se puede solucionar con un reverse proxy.
